    ...
    def _lookup_conceptnet_concept_ids(self, sub_tokens, sub_to_ori_index, tokens, tolower, tokenizer):
        ...
        for index in range(len(sub_tokens)):
            ...
            if retrieve_token in self.synsets_info:
                ...
                weights.append(
                    [self.cn_concept2id[synset['weight']] for synset in self.synsets_info[retrieve_token]])
            else:
                concept_ids.append([])
                weights.append([])
    ...
    def __call__(self,
        ...
        for example in tqdm(examples):
            ...
            for cindex in range(len(concept_ids)):
                ...
                concept_weights[cindex] = cn_concept_weights[cindex]+[0]*(max_concept_length - len(concept_ids[cindex]))
...
class DataProcessor(object):
    ...
    def data_generator(self, data_path, batch_size, phase, **concept_settings):
        ...
        def batch_reader(features, batch_size):
            batch,input_ids,token_type_ids,attention_mask,cn_concept_ids,label = [],[],[],[],[],[]
            batch_data = {}
            for (index, feature) in enumerate(features):
                ...
                if to_append:
                    ...
                    batch_data = {}
                    input_ids, token_type_ids, attention_mask, cn_concept_ids, nell_concept_ids, label = [], [], [], [], [], []
            return batch
