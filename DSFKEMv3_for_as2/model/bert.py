...
class BertClassfication(nn.Module):
    def __init__(self,args,cn_concept_embedding_mat):
        super(BertClassfication, self).__init__()
        self.model_name = args.pre_trained_model
        self.model = BertModel.from_pretrained(self.model_name)
        self.tokenizer = BertTokenizer.from_pretrained(self.model_name)
        self.fc = nn.Linear(1068, 2)  # 768取决于BERT结构，2-layer, 768-hidden, 12-heads, 110M parameters
        ...
        self.cn_memory_layer = MemoryLayer(bert_size=768, mem_emb_size=cn_concept_embedding_mat.shape[1], mem_method='raw')
    ...
    def forward(self,
        ...
        cn_concept_ids_reduced = cn_concept_ids > 0  #[batch_size, seq_size, wn_concept_size]
        cn_concept_ids_reduced = cn_concept_ids_reduced.float()  #[batch_size, seq_size, wn_concept_size]
        cn_mem_length = cn_concept_ids_reduced.sum(dim=2)  #[batch_size, seq_size]
        ...
        '''
        cn_memory_output = self.cn_memory_layer(bert_output=enc_out, memory_embs=cn_memory_embs, mem_length=cn_mem_length,  ignore_no_memory_token="True", concept_size=max_cn_concept_length)
        # [batch_size, seq_size, mem_emb_size]
        #nell_memory_output = self.nell_memory_layer(bert_output=enc_out, memory_embs=nell_memory_embs, mem_length=nell_mem_length,  ignore_no_memory_token="True", concept_size=max_nell_concept_length)
        # [batch_size, seq_size, mem_emb_size]
