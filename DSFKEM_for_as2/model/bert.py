...
class BertClassfication(nn.Module):
    def __init__(self,args,wn_concept_embedding_mat,nell_concept_embedding_mat):
        ...
        self.fc = nn.Linear(968, 2)  # 768取决于BERT结构，2-layer, 768-hidden, 12-heads, 110M parameters
        ...
        '''
        attn = MultiHeadedAttention(11, 968)
        ff = PositionwiseFeedForward(968, 968, 0.2)
        self.encoder = Encoder(EncoderLayer(968, deepcopy(attn), deepcopy(ff), 0.2), 1)
    ...
    def forward(self,
        ...
        wn_memory_output = self.wn_memory_layer(bert_output=enc_out, memory_embs=wn_memory_embs, mem_length=wn_mem_length,  ignore_no_memory_token="True", concept_size=max_wn_concept_length)
