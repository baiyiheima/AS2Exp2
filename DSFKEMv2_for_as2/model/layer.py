...
class MemoryLayer(nn.Module):
    ...
    def forward(self, bert_output, memory_embs, mem_length,  ignore_no_memory_token, concept_size):
        ...
        expanded_bert = torch.unsqueeze(projected_bert, 1)  # [batch_size, 1, seq_size, mem_emb_size]
        expanded_bert = expanded_bert.repeat(1,100,1,1)  # [batch_size,seq_size,seq_size,mem_emb_size]

        extended_memory, memory_score, kg_prior_att = self.add_sentinel(expanded_bert, memory_embs, self.mem_emb_size)
        ...
        memroy_score_top_k = torch.topk(input=memory_score,k=int((concept_size+1)/2),dim=2).values  # [batch_size,sqe_size,k]
        memroy_score_top_k = memroy_score_top_k[:,:,-1]  # [batch_size,sqe_size]
        memroy_score_top_k = torch.unsqueeze(memroy_score_top_k,2)  # [batch_size,sqe_size,1]
        memroy_score_top_k = memroy_score_top_k.repeat(1,1,concept_size+1)  # [batch_size,sqe_size,concept_size+1]
    ...
    def add_sentinel(self, expanded_bert, memory_embs, mem_emb_size):
        ...
        memory_score = torch.matmul(expanded_bert,extended_memory)  # [batch_size, seq_size, seq_size, 1+concept_size]
        kg_prior_att = torch.mean(memory_score,3)
        ...
        return extended_memory, memory_score, kg_prior_att
